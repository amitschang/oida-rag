...
use crate::ingest;
use crate::model::{Artifact, RawArtifact};
...
impl Index {
    ...
    pub async fn is_ingested(config: &CoreConfig) -> bool {
        if !config.lance_path.exists() {
            return false;
        }
    ...
    /// Upsert already-built `documents`/`artifacts` batches into the index.
    ///
    /// Documents are merged on `id` (insert-or-replace). For each of `ids` the
    /// previous artifact rows are deleted and the incoming set re-appended, so a
    /// changed content fingerprint is reflected exactly. The batches are the same
    /// mapping the full build produces (sliced to the changed rows), so the
    /// incremental path reuses the provider's mapping — no second mapper.
    /// Callers should batch per source page to bound memory.
    pub(crate) async fn upsert_documents(
        &self,
        documents: RecordBatch,
        ...
        let schema = documents.schema();
        let reader = RecordBatchIterator::new(vec![Ok(documents)], schema);
    ...
    /// Map each of `ids` to its stored `documents.digest` fingerprint.
    ///
    /// The incremental apply compares this — a plain string, zero field
    /// knowledge — against the digest the provider computes for the incoming
    /// row: unequal ⇒ some upsert-worthy field (metadata or artifact set)
    /// changed. Ids not in the index are simply absent from the map.
    pub async fn document_digests(
        &self,
        ids: &[String],
        ...
        let batches: Vec<RecordBatch> = self
            ...
            .select(Select::columns(&["id", "digest"]))
            .execute()
            .await
            .context("executing document-digests query")?
            .try_collect()
            .await
            .context("collecting document digests")?;
        for batch in &batches {
            let id = str_col(batch, "id")?;
            let digest = str_col(batch, "digest")?;
            for i in 0..batch.num_rows() {
                if !id.is_null(i) && !digest.is_null(i) {
                    map.insert(id.value(i).to_string(), digest.value(i).to_string());
                }
            }
        }
        Ok(map)
    ...
    /// Open the `raw_artifacts` blob table if it has been built, for the
    /// serving-time [`crate::source::ArtifactReader`] resolver. Returns `None`
    /// when raw storage was never materialized.
    pub async fn open_raw_table(&self) -> Result<Option<Table>> {
    ...
    /// optionally capped at `limit` rows, decoding each row as `D`.
    ///
    /// The generic filtered-projection primitive domain queries (Bates lookups,
    /// conversation threads) build on from outside the framework.
    pub async fn documents_where<D: DocumentRow>(
        &self,
        filter: &str,
        limit: Option<usize>,
    ) -> Result<Vec<D>> {
        ...
        collect_rows::<D>(&batches)
    }

    /// Fetch one document by `id`, decoded as the provider's row type `D`.
    ///
    /// Projects only `D::columns()`, so a lean result type scans fewer columns.
    pub async fn get<D: DocumentRow>(&self, id: &str) -> Result<Option<D>> {
        let batches: Vec<RecordBatch> = self
            .documents
            .query()
            ...
            .execute()
            .await
            .context("executing get query")?
            .try_collect()
            .await
            .context("collecting get result")?;
        Ok(collect_rows::<D>(&batches)?.into_iter().next())
    }

    /// Fetch documents for `ids`, decoded as `D`, preserving the order of `ids`.
    /// Ids with no matching document are skipped. Used to hydrate ranked
    /// hybrid-search results back into document metadata.
    pub async fn get_many<D: DocumentRow>(&self, ids: &[String]) -> Result<Vec<D>> {
        if ids.is_empty() {
            return Ok(Vec::new());
        }
        let list = ids.iter().map(|i| sql_str(i)).collect::<Vec<_>>().join(", ");
        let batches: Vec<RecordBatch> = self
            ...
            .execute()
            .await
            .context("executing get_many query")?
            .try_collect()
            .await
            .context("collecting get_many result")?;
        // `IN` does not preserve order; reorder to match `ids`.
        let mut by_id: std::collections::HashMap<String, D> = collect_rows::<D>(&batches)?
            .into_iter()
    ...
    /// Full-text search `search_text`, decoding candidates as `D` (projected to
    /// `D::columns()`). Powers the generic metadata [`Index::search`].
    pub(crate) async fn documents_fts_rows<D: DocumentRow>(
        &self,
        query: &str,
        limit: usize,
    ) -> Result<Vec<D>> {
...
pub fn track_max(cur: &mut Option<String>, candidate: String) {
...
/// Decode every row across `batches` into the provider's row type `D`.
///
/// Runs only at result materialization (a handful of rows per query), so the
/// per-call column re-downcasts inside `D::from_row` are off any hot path.
pub fn collect_rows<D: DocumentRow>(batches: &[RecordBatch]) -> Result<Vec<D>> {
    let mut out = Vec::new();
    for batch in batches {
        for row in 0..batch.num_rows() {
            out.push(D::from_row(batch, row)?);
        }
    }
    Ok(out)
}
...
/// Row-level decode helper for [`DocumentRow::from_row`] impls.
pub fn row_opt_str(batch: &RecordBatch, name: &str, row: usize) -> Result<Option<String>> {
    Ok(opt_at(str_col(batch, name)?, row))
}

/// Read a `List<Utf8>` cell into a `Vec<String>`, dropping null/empty elements.
/// Row-level decode helper for [`DocumentRow::from_row`] impls.
pub fn row_list(batch: &RecordBatch, name: &str, row: usize) -> Result<Vec<String>> {
    list_at(batch, name, row)
}

/// Read an `Int64` cell as `u64` (null → 0, negatives clamped to 0).
/// Row-level decode helper for [`DocumentRow::from_row`] impls.
pub fn row_u64(batch: &RecordBatch, name: &str, row: usize) -> Result<u64> {
    let col = i64_col(batch, name)?;
    Ok(if col.is_null(row) {
        0
    } else {
        col.value(row).max(0) as u64
    })
}
