...
use crate::ollama::{ChatMessage, Ollama, ToolFunctionInfo, ToolInfo, ToolType};
...
impl Agent {
    /// Build an agent from a configured Ollama endpoint and MCP tool list.
    pub fn new(
        ollama_host: &str,
        model: String,
        mcp: McpClient,
    ...
    pub fn new_history(&self) -> Vec<ChatMessage> {
        vec![ChatMessage::system(SYSTEM_PROMPT.to_string())]
    }
