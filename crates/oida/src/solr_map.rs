...
/// The *policy* redaction signal for a document: withdrawn at the source
/// because it is deaccessioned or unpublished. This is the one genuinely
/// domain signal the [`crate::provider::SourcePage`] carries; the framework
/// derives *structural* redaction ("no artifacts") itself.
pub(crate) fn is_redacted_policy(doc: &Value) -> bool {
    let flagged = |field: &str, redacted_when: bool| {
        matches!(doc.get(field), Some(Value::Bool(b)) if *b == redacted_when)
    };
    flagged("deaccessioned", true) || flagged("published", false)
}
